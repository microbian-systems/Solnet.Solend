...
{
    public class RedeemCollateralToken : IRunnableExample
    {
        ...
        public async void Run()
        {
            ...
            // the process below assumes that in case the mint is NOT wrapped SOL, that a token account with enough balance to redeem 1 cToken exists
            ...
            // we want to redeem 1 cSOL
            var nativeRedeemAmount = 0ul;

            if (tokenMint == WellKnownTokens.WrappedSOL.TokenMint)
            {
                nativeRedeemAmount = SolHelper.ConvertToLamports(1m);
            }
            else
            {
                nativeRedeemAmount = (ulong)(1m * (decimal)Math.Pow(10, reserve.Liquidity.Decimals));
            }
            ...
            // OR in case the necessary liquidity token account doesn't exist
            ...
            txBuilder
                .AddInstruction(Solend.RefreshReserve(
                    new(reservePubkey),
                    reserve.Liquidity.PythOracle,
                    reserve.Liquidity.SwitchboardOracle))
                .AddInstruction(Solend.RedeemReserveCollateral(
                    nativeRedeemAmount,
                    collateralAta,
                    liquidityAta,
                    new(reservePubkey),
