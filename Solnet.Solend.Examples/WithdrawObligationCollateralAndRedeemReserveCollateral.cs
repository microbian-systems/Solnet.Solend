...
{
    public class WithdrawObligationCollateralAndRedeemReserveCollateral : IRunnableExample
    {
        ...
        public WithdrawObligationCollateralAndRedeemReserveCollateral()
        {
        ...
        public async void Run()
        {
            ...
            // check if we're withdrawing wrapped sol so that we wrap SOL before depositing
            if (tokenMint == WellKnownTokens.WrappedSOL.TokenMint)
            {
                txBuilder
            ...
            txBuilder.AddInstruction(Solend.RefreshObligation(
                    obligationPubkey,
                    obligationReserves))
                .AddInstruction(Solend.WithdrawObligationCollateralAndRedeemReserveCollateral(
            ...
            // in case we're withdrawing SOL, close the wrapped SOL ata
            if (tokenMint == WellKnownTokens.WrappedSOL.TokenMint)
            {
                txBuilder
