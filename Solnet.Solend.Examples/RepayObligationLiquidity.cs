...
{
    public class RepayObligationLiquidity : IRunnableExample
    {
        ...
        public RepayObligationLiquidity()
        {
        ...
        public async void Run()
        {
            ...
            // the process below assumes that a wallet with enough balance to repay a liquidity borrow of 1 SOL exists
            ...
            var nativeRepayAmount = 0ul;

            if (tokenMint == WellKnownTokens.WrappedSOL.TokenMint)
            {
                nativeRepayAmount = SolHelper.ConvertToLamports(1m);
            }
            else
            {
                nativeRepayAmount = (ulong)(1m * (decimal)Math.Pow(10, reserve.Liquidity.Decimals));
            }
            ...
            // check if we're repaying SOL so that we create wrapped SOL ata before repaying
            if (tokenMint == WellKnownTokens.WrappedSOL.TokenMint)
            {
                txBuilder
                    .AddInstruction(SystemProgram.Transfer(
                        Wallet.Account,
                        liquidityAta,
                        nativeRepayAmount + tokenAccountRent.Result
                        ))
            ...
            // refresh the obligation and repay 1 SOL
            txBuilder.AddInstruction(Solend.RefreshObligation(
                    obligationPubkey,
                    obligationReserves))
                .AddInstruction(Solend.RepayObligationLiduidity(
                    nativeRepayAmount,
                    liquidityAta,
